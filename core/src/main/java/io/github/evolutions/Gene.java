...
public class Gene {
    ...
    public static class gene_builder {
        ...
        byte mutation_volatility = (byte) (random_travel_range / 20);      // this means the value can change 5% of the travel range units per round of mutation             
    ...
    private byte mutate(byte current_gene_value) {
        if (Math.abs(GeneUtils.getRandomGeneByte((byte) 100)) <= this.mutation_probability) {  // calculate if mutation is going to happen using mutation probability
            ...
            mutation_sum = (byte) (Math.max(mutation_sum, -this.random_travel_range));           // check if mutated gene is within travel bounds
            mutation_sum = (byte) (Math.min(mutation_sum, this.random_travel_range));

            if (mutation_sum == 0) {
                return GeneUtils.getRandomGeneByte((byte) 0b01);
            }
            else {
                return mutation_sum;
            }
